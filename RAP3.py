...
class VideoProcessor:
    def __init__(self, video_source=0, output_dir="output"):
        ...
        self.face_encodings = {}  # Store face encodings for re-identification
        self.person_face_mapping = {}  # Map person IDs to face encodings
        
        # Re-identification variables
        self.known_faces = {}
        self.face_similarity_threshold = 0.6
        
        # Clean up None values periodically
        self.cleanup_counter = 0
        
        # Visualization variables
        self.tracking_history = defaultdict(lambda: deque(maxlen=30))  # Store last 30 positions
    ...
    def cleanup_known_faces(self):
        """Remove None values from known_faces dictionary"""
        self.known_faces = {k: v for k, v in self.known_faces.items() if v is not None}
    
    def detect_persons(self, frame):
        """Step 1: Person Detection using YOLOv5"""
    ...
    def extract_face_encoding(self, face_roi):
        """Extract face encoding for re-identification"""
        try:
            # Check if face_roi is valid
            if face_roi is None or face_roi.size == 0:
                return None
                
            # Ensure minimum size for face encoding
            ...
            # Only print error if it's not a common face_recognition error
            if "No faces found" not in str(e):
                print(f"Error extracting face encoding: {e}")
        return None
    
    def reidentify_person(self, face_encoding):
        """Step 3: Re-identification using face encoding"""
        ...
        for person_id, known_encoding in self.known_faces.items():
            try:
                # Check if known_encoding is valid
                ...
                # Only print error if it's not a NoneType error (to reduce spam)
    ...
    def track_persons(self, current_detections):
        """Step 4: Person Tracking with IoU and distance matching"""
        used_detections = set()
        new_tracked_people = {}
        ...
        for person_id, track_info in self.tracked_people.items():
            ...
            if best_match is not None:
                ...
                # Keep tracking for a few frames
                new_tracked_people[person_id] = {
                    'bbox': prev_bbox,
    ...
    def visualize_results(self, frame, tracked_people):
        """Step 5: Visualization with tracking history and labels"""
        for person_id, track_info in tracked_people.items():
            ...
            if disappeared > 0:
                color = (0, 165, 255)  # Orange for temporarily lost
                alpha = 0.5
            else:
    ...
    def export_results(self, frame, tracked_people):
        """Step 6: Export results (save frames, faces, and detection log)"""
        # Save frame
        frame_path = os.path.join(self.output_dir, "frames", f"frame_{self.frame_count:06d}.jpg")
        cv2.imwrite(frame_path, frame)
        
        # Save faces and update detection log
        frame_detections = []
        for person_id, track_info in tracked_people.items():
            if track_info['disappeared'] == 0:  # Only active detections
                detection_info = {
    ...
    def process_video(self):
        ...
        while True:
            ...
            # Step 2: Face Detection for each person
            for detection in person_detections:
                ...
                if faces:
                    ...
                    if matched_id is not None:
                        ...
                        # New person - store face encoding only if it's valid
            ...
            # Step 6: Export (every 30 frames to avoid too many files)
...
def main():
    ...
    # You can change the video source here:
    # - Use 0 for webcam
    # - Use a file path for video file: "path/to/video.mp4"
